package me.chentao.redpacket.ui.widget;
...
/**
 * fix {@link MaterialHeader#autoOpen(int, float, boolean)} 中 onAnimationEnd 方法回调中的空指针异常
 * <br>
 * create by chentao on 2023-11-10.
 */
public class MaterialHeaderCompat extends SimpleComponent implements RefreshHeader {
  ...
  public MaterialHeaderCompat(Context context) {
    this(context, null);
  }

  public MaterialHeaderCompat(Context context, AttributeSet attrs) {
    ...
    TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MaterialHeaderCompat);

    mShowBezierWave = ta.getBoolean(R.styleable.MaterialHeaderCompat_srlShowBezierWave, mShowBezierWave);
    mScrollableWhenRefreshing = ta.getBoolean(R.styleable.MaterialHeaderCompat_srlScrollableWhenRefreshing, mScrollableWhenRefreshing);
    mBezierPaint.setColor(ta.getColor(R.styleable.MaterialHeaderCompat_srlPrimaryColor, 0xff11bbff));
    if (ta.hasValue(R.styleable.MaterialHeaderCompat_srlShadowRadius)) {
      int radius = ta.getDimensionPixelOffset(R.styleable.MaterialHeaderCompat_srlShadowRadius, 0);
      int color = ta.getColor(R.styleable.MaterialHeaderCompat_mhShadowColor, 0xff000000);
      mBezierPaint.setShadowLayer(radius, 0, 0, color);
      thisView.setLayerType(LAYER_TYPE_SOFTWARE, null);
    }

    mShowBezierWave = ta.getBoolean(R.styleable.MaterialHeaderCompat_mhShowBezierWave, mShowBezierWave);
    mScrollableWhenRefreshing = ta.getBoolean(R.styleable.MaterialHeaderCompat_mhScrollableWhenRefreshing, mScrollableWhenRefreshing);
    if (ta.hasValue(R.styleable.MaterialHeaderCompat_mhPrimaryColor)) {
      mBezierPaint.setColor(ta.getColor(R.styleable.MaterialHeaderCompat_mhPrimaryColor, 0xff11bbff));
    }
    if (ta.hasValue(R.styleable.MaterialHeaderCompat_mhShadowRadius)) {
      int radius = ta.getDimensionPixelOffset(R.styleable.MaterialHeaderCompat_mhShadowRadius, 0);
      int color = ta.getColor(R.styleable.MaterialHeaderCompat_mhShadowColor, 0xff000000);
      mBezierPaint.setShadowLayer(radius, 0, 0, color);
      thisView.setLayerType(LAYER_TYPE_SOFTWARE, null);
    }
   ...
   */
  public MaterialHeaderCompat setProgressBackgroundColorSchemeResource(@ColorRes int colorRes) {
    final View thisView = this;
   ...
   * @param color 颜色
   */
  public MaterialHeaderCompat setProgressBackgroundColorSchemeColor(@ColorInt int color) {
   ...
   */
  public MaterialHeaderCompat setColorSchemeColors(@ColorInt int... colors) {
   ...
   * @return MaterialHeader
   */
  public MaterialHeaderCompat setColorSchemeResources(@ColorRes int... colorIds) {
    final View thisView = this;
    final Context context = thisView.getContext();
   ...
   * @return MaterialHeader
   */
  public MaterialHeaderCompat setSize(int size) {
   ...
   * @return MaterialHeader
   */
  public MaterialHeaderCompat setShowBezierWave(boolean show) {
   ...
   */
  public MaterialHeaderCompat setScrollableWhenRefreshing(boolean scrollable) {
